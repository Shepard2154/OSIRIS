...
function rebuildBoard(){
	selection = document.getElementById('selection123')
	datta = String('{"ListInfluencersName":"'+selection.value+'"}')
        $.ajax({
            ...
            success: function(msg) {
				$("#messageBoard").html("");
				sizze = Object.keys(msg.Influencers).length
                if (sizze > 0) {
                    for (var i = 0; i < sizze; i++) {
                        addPersonOnBoard(msg.Influencers[i]);
					}
					$("#section2").attr("style","visibility: true;")
					if (msg.Followers == true){
						$("#filterFollowers").attr("style","visibility: true;")
					}
					if (msg.Followers == false){
						$("#filterFollowers").attr("style","visibility: hidden;")
					}
				}
				else {
					$("#messageBoard").html('<div id="imageEmptyList" class="p-4 w-full lg:w-1/2 xl:w-1/3" style="width: 25%;"><img src="../static/images/nothing.png"></div>');
					$("#section2").attr("style","visibility: hidden;")		
				}
				statUpdate()
            },
            error: function(msg) {
	...
	datta = String('{"ListCaName":"'+selection.value+'"}')
        $.ajax({
            ...
            success: function(msg) {
				...
				// $("#messageBoard").html("");
				sizze = Object.keys(msg.Ca).length
				alert(sizze)
                // if (sizze > 0) {
                //     for (var i = 0; i < sizze; i++) {
                //         addPersonOnBoard(msg.Influencers[i]);
