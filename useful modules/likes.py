...
def get_likes(USER, n_posts):
  tweet_list=api.favorites(screen_name = USER, count = n_posts)
  usr = api.get_user(USER)
  likes = []
  for i in tweet_list:
    likes.append({'liker' : USER, 'liker_id' : usr.id ,'liked_user' : i.user.screen_name, 'liked_user_id' : i.user.id, 'tweet_text' : i.text , 'tweet_hashtags' : re.findall(r'(#\w+)', i.text), 'tweet_links' : re.findall("(?P<url>https?://[^\s]+)", i.text)})
  return(likes)